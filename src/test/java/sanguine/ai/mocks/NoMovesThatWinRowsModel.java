...
/**
 * Mocked model that has no possible moves that win a row so AI passes turn.
 */
public class NoMovesThatWinRowsModel implements SanguineModel {
   ...
   */
  public NoMovesThatWinRowsModel(Appendable log) throws IllegalArgumentException {
    if (log == null) {
  ...
  public List<Card> getCurrentHand() throws IllegalStateException {
    ...
    for (int cardIdx = 0; cardIdx < 3; cardIdx++) {
      Card card = new SanguineCard("Filler", 0, 0, null, Team.RED);
      hand.add(card);
    }
